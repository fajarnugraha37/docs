# Strict Coding Standards — Java 25

> **Target runtime:** Java 25 / Java SE 25  
> **Audience:** LLM code agents, human reviewers, maintainers  
> **Purpose:** make generated Java 25 code predictable, reviewable, secure, maintainable, and compatible with a strict Java 25 baseline.
...
1. Preserve Java 25 compatibility.
2. Use only Java features and APIs available in **stable Java 25** unless the repository explicitly enables preview/incubator/experimental features.
...
6. Never introduce dependencies, frameworks, runtime assumptions, public API changes, persistence changes, serialization changes, thread model changes, native interop, startup/AOT assumptions, or behavioral changes without clear justification.
...
11. Distinguish **stable Java 25 features** from **preview/incubator/experimental Java 25 features**.
12. Treat Java 25 as an LTS production baseline, not a playground for every new feature.

The agent **MUST NOT**:

1. Use Java features introduced after Java 25.
2. Use Java 25 preview, incubator, or experimental features unless the user explicitly requests them and the build already enables them.
3. Generate code that only works on Java 26 or newer.
...
7. Add concurrency, caching, async behavior, virtual threads, scoped values, reflection, serialization, bytecode manipulation, global state, native interop, AOT configuration, dynamic agents, or background scheduling unless the requirement truly needs it.
...
11. Use compact source files, module imports, unnamed variables, stream gatherers, scoped values, or FFM just because they are available in Java 25.

---

## 1. Java 25 compatibility contract

### 1.1 Build level

All Java source code **MUST** compile with Java 25.
...
The agent **MUST NOT** rely only on `sourceCompatibility = 25` and `targetCompatibility = 25` when `--release 25` is available, because `--release` prevents accidental use of newer JDK APIs during compilation.

### 1.2 Java 25 allowed feature surface

The following Java 25-compatible features and APIs are allowed when they improve clarity.
...
| UTF-8 default charset behavior                     |           18 | Awareness          | Still specify charset explicitly for protocols, files, persistence, signatures, and logs.               |
...
| Foreign Function & Memory API                      |           22 | Restricted         | Use only for approved native interop with strict memory lifetime boundaries.                            |
| Unnamed variables and patterns                     |           22 | Restricted         | Use only for intentionally ignored locals/patterns; prefer meaningful names when meaning matters.       |
| Launch multi-file source-code programs             |           22 | Tooling only       | Useful for scripts/prototypes; do not use as production module structure.                               |
| Markdown documentation comments                    |           23 | Restricted         | Use only if repository Javadoc tooling and style allow it.                                              |
| Stream gatherers                                   |           24 | Restricted         | Use only when a custom intermediate stream operation is clearer and tested.                             |
| Class-File API                                     |           24 | Restricted         | Build/tooling only; never for ordinary business logic.                                                  |
| Ahead-of-Time class loading/linking                |           24 | Runtime/deployment | Do not hard-code source behavior around AOT caches.                                                     |
| Module import declarations                         |           25 | Restricted         | Avoid in production/shared source; acceptable for scripts/examples where import breadth is intentional. |
| Compact source files and instance main methods     |           25 | Restricted         | Use only for small scripts, examples, learning, or throwaway utilities; not production services.        |
| Flexible constructor bodies                        |           25 | Restricted         | Use for pre-`super` validation/preparation only; do not access the object under construction illegally. |
| Scoped values                                      |           25 | Restricted         | Use for immutable, bounded, request-scoped context where explicit parameters are impractical.           |
| Key Derivation Function API                        |           25 | Restricted         | Use only inside reviewed cryptographic design. Do not invent password/secret derivation schemes.        |
| Compact object headers                             |           25 | Runtime option     | Runtime tuning only; never make source-level identity/layout assumptions.                               |
| Generational Shenandoah                            |           25 | Runtime option     | Runtime tuning only; never hard-code GC assumptions into source code.                                   |
| AOT command-line ergonomics / AOT method profiling |           25 | Runtime/deployment | Deployment optimization only; source code must remain correct without AOT.                              |
| JFR cooperative sampling / method timing/tracing   |           25 | Observability      | Profiling/diagnostics only; avoid business logic dependence.                                            |

### 1.3 Java 25 preview/incubator/experimental features are forbidden by default

Java 25 includes preview, incubator, and experimental features. Generated code **MUST NOT** use them by default.

Forbidden unless explicitly requested and build-enabled:

1. PEM Encodings of Cryptographic Objects — preview API.
2. Stable Values — preview API.
3. Structured Concurrency — fifth preview API.
4. Primitive Types in Patterns, `instanceof`, and `switch` — third preview language feature.
5. Vector API — tenth incubator API.
6. JFR CPU-Time Profiling — experimental.
7. Any API requiring `--enable-preview`, incubator modules, or experimental flags.
8. Any Java 26 feature, including features that continue from Java 25 previews.

If the user explicitly requests preview/incubator/experimental usage, the agent **MUST**:

1. State that the code is no longer strict portable Java 25 baseline.
...
4. Keep preview/incubator code isolated behind a small boundary.
...
7. Treat preview APIs as unstable even when the concept has existed for multiple releases.

### 1.4 Forbidden Java >25 features

The agent **MUST NOT** use these in strict Java 25 code:

| Forbidden feature/API                                   | Introduced after Java 25 | Replacement in Java 25                                                                   |
| ------------------------------------------------------- | -----------------------: | ---------------------------------------------------------------------------------------- |
| HTTP/3 for the HTTP Client API                          |                       26 | HTTP/1.1 or HTTP/2 using `java.net.http.HttpClient`, or project-approved client.         |
| Prepare to Make Final Mean Final behavior assumptions   |                       26 | Do not mutate final fields reflectively; avoid relying on final-field mutation.          |
| Remove Applet API assumptions                           |                       26 | Do not use applets at all.                                                               |
| Lazy Constants / later Stable Values evolution          |              26+ preview | Explicit initialization, memoization, or Java 25 Stable Values only if preview approved. |
| Structured Concurrency sixth preview or later API shape |                      26+ | Java 25 fifth-preview API only if preview approved; otherwise normal executors.          |
| Vector API eleventh incubator or later                  |                      26+ | Java 25 vector API only if incubator approved; otherwise scalar loops/SIMD-neutral code. |
| Any Valhalla value classes / primitive classes          |                   future | Normal classes, records, and primitives.                                                 |
| Any Java 26+ language syntax                            |                      26+ | Stable Java 25 syntax only.                                                              |

### 1.5 Removed, deprecated, or changed Java 25 platform behavior

The agent **MUST NOT** assume removed, obsolete, or restricted JDK components exist.

Java 25-relevant guardrails:

1. The 32-bit x86 port is removed. Do not generate deployment assumptions for 32-bit x86.
2. The Windows 32-bit x86 port was removed before Java 25 and remains unavailable.
3. The optional experimental Graal JIT compiler has been removed from Oracle JDK 25.
4. `SecurityManager` has been permanently disabled since JDK 24; do not design security boundaries around it.
5. ZGC non-generational mode was removed before Java 25; do not use old non-generational ZGC flags.
6. Strong encapsulation of JDK internals remains enforced; do not use `sun.*`, `com.sun.*`, reflective access to JDK internals, or `--add-opens` as a lazy fix.
7. The memory-access methods in `sun.misc.Unsafe` are deprecated for removal and produce warnings; do not generate new code depending on them.
8. JNI usage is being prepared for restriction; avoid adding JNI/native access unless the project explicitly owns the native boundary.
9. Dynamic agent loading has been warned about since Java 21 and should not be introduced casually.
10. Finalization is deprecated for removal; do not add finalizers.
11. RMI Activation was removed; do not generate code depending on `rmid` or activation APIs.
12. Nashorn JavaScript engine was removed before Java 25; do not generate new code using Nashorn.
13. Pack200 was removed before Java 25; do not generate Pack200-based packaging logic.
14. Java EE/CORBA modules removed from JDK 11 remain absent; use explicit dependencies only when the project already owns them.
15. JavaFX is not bundled with the JDK; do not assume it exists.
16. Applet APIs are obsolete and should not be used.
17. `java.net.Socket` constructors with `stream=false` must not be used to create datagram sockets; use `DatagramSocket`.
18. On Linux, `-Djdk.lang.Process.launchMechanism=VFORK` is deprecated; do not introduce it.
19. Do not rely on default charset for persistent formats even though modern JDKs standardize UTF-8 by default; explicitly pass `StandardCharsets.UTF_8`.

### 1.6 Stable vs restricted vs forbidden rule

For Java 25 code generation, every feature falls into one of three operational buckets:

1. **Allowed** — normal Java feature/API that can be used when it improves clarity.
2. **Restricted** — stable but easy to misuse; use only when the invariant justifies it.
3. **Forbidden by default** — preview/incubator/experimental/unsupported; use only with explicit approval and build flags.

The agent **MUST** prefer stable boring constructs over restricted constructs when the restricted construct does not encode a real invariant.
...
16. Java 25 adoption policy, especially for virtual threads and pattern matching.

If the repository convention conflicts with this document:

1. Existing repository convention wins for local consistency.
2. Java 25 compatibility always wins over repository inconsistency.
...
## 6. Modern Java 25 language and platform features

Java 25 gives the agent many more tools than Java 11/17/21. That does **not** mean the agent should use them automatically.

The default rule is:

> Use modern Java features only when they make the model clearer, the invariant stronger, or the failure behavior easier to review.
...
7. Do not convert existing public DTOs to records without checking serialization compatibility.
8. Do not use records for framework proxies unless the framework explicitly supports them.
...
Record patterns are stable and allowed, but **restricted**.
...
5. Treat pattern variables as read-only even though Java does not make them implicitly final.
6. Do not combine record patterns with primitive pattern preview features in strict Java 25 baseline.

### 6.6 Pattern matching for `switch`

Pattern matching for `switch` is stable, but **restricted**.
...
11. Do not use primitive type patterns in `switch`; they are preview in Java 25 and forbidden by default.
...
Sequenced collections may be used when encounter order is part of the contract.
...
Virtual threads are stable, but **restricted**.
...
5. The repository or runtime platform has explicitly adopted virtual-thread execution.
...
1. **Never pool virtual threads.**
2. Do not use virtual threads for CPU-bound speedup.
3. Bound database connections, HTTP connections, rate limits, and downstream capacity separately.
4. Preserve timeout, cancellation, and interruption behavior.
5. Avoid thread-local-heavy designs.
6. Prefer scoped values or explicit context over broad thread-local context only when the context invariant justifies it.
7. Do not assume every library behaves well under virtual-thread load.
8. Benchmark and load test before changing a service execution model.
9. Avoid pinning patterns where possible; be careful with long synchronized blocks around blocking I/O.
10. Keep executor lifetime explicit with try-with-resources.

Bad:

```java
Executors.newFixedThreadPool(100, Thread.ofVirtual().factory()); // do not pool virtual threads
```

### 6.11 Scoped values

Scoped values are stable in Java 25, but **restricted**.

Use scoped values only when:

1. The data is immutable or effectively immutable.
2. The data has a clear bounded lifetime.
3. The data is read by callees as contextual information.
4. Passing parameters through every layer is impractical or would expose framework internals.
5. The scope is lexically obvious.
6. The context is not business state that should be explicit in the domain model.
...
final class RequestContextHolder {
    ...
    static void runWith(RequestContext context, Runnable operation) {
        Objects.requireNonNull(context, "context");
        Objects.requireNonNull(operation, "operation");
        ScopedValue.where(REQUEST_CONTEXT, context).run(operation);
    }

    static RequestContext current() {
        if (!REQUEST_CONTEXT.isBound()) {
            throw new IllegalStateException("request context is not bound");
        }
...
1. Declare scoped values as `private static final` unless intentionally shared inside a package/framework boundary.
2. Do not bind mutable objects unless they are immutable by contract.
3. Do not use scoped values as hidden method parameters for core domain logic.
4. Do not use scoped values for transaction state unless the framework owns that lifecycle.
5. Do not use scoped values as a global service locator.
6. Prefer explicit parameters for data that is part of the business invariant.
7. Always expose a small wrapper boundary instead of scattering `ScopedValue.where(...)` across business code.
8. In tests, bind scoped values explicitly and keep scope minimal.
9. Do not mix `ThreadLocal` and `ScopedValue` for the same context unless migrating with a clear plan.
10. Document the lifetime and ownership of every shared scoped value.

### 6.12 Unnamed variables and patterns

Unnamed variables and patterns are stable, but **restricted**.

Use `_` only when the variable is intentionally ignored and naming it would add noise.
...
1. Use `_` only when the value is intentionally unused.
2. Do not use `_` to hide missing business logic.
3. Prefer a meaningful name when the ignored value explains the code.
4. Do not use `_` in public examples where readers need semantic clarity.
5. Do not use `_` in logging/error handling when the exception cause should be preserved.
6. Do not use `_` to silence warnings without understanding the ignored value.

Bad:

```java
catch (Exception _) {
    return null; // exception swallowed and cause lost
}
```

### 6.13 Module import declarations

Module import declarations are stable in Java 25, but **restricted**.
...
> Do not use `import module` in production service/library code unless the repository already standardizes it.
...
4. JShell-style exploratory code.
5. Internal one-off utilities where broad imports are intentional.

Rules:

1. Prefer explicit single-type imports in mature codebases.
2. Do not use module imports to hide dependency boundaries.
3. Do not combine many module imports in normal production files.
4. Avoid module imports in public APIs, generated code, and shared libraries.
5. Avoid if ambiguous simple names become likely.
6. Do not require the project to become modular only to use this feature.

### 6.14 Compact source files and instance main methods

Compact source files and instance main methods are stable in Java 25, but **restricted**.

Allowed:

1. Small scripts.
2. Learning examples.
3. Temporary admin utilities.
4. Reproducers.
5. Local experiments.

Forbidden by default in:

1. Production services.
2. Shared libraries.
3. Domain modules.
4. Framework-managed applications.
5. Code that must follow normal package/class layout.

Bad for production service code:
...
Production code **MUST** keep explicit package, class, and entrypoint structure unless the repository intentionally uses compact source files.

### 6.15 Flexible constructor bodies

Flexible constructor bodies are stable in Java 25, but **restricted**.

Use them to validate or prepare values before calling `super(...)` or `this(...)`.
...
1. Use only for validation, normalization, or local computation needed for delegation.
2. Do not access instance methods, `this`, or object state before superclass construction.
3. Keep pre-constructor logic short.
4. Extract complex validation into static helper methods.
5. Do not add side effects before `super(...)` unless failure semantics are explicitly intended.
6. Do not perform I/O, database access, network calls, thread creation, logging side effects, or dependency lookup before `super(...)`.
7. Preserve constructor invariants and failure atomicity.
...
class Child extends Parent {
    Child(String value) {
        auditService.record(value); // side effect before object construction; avoid
        super(value);
    }
}
```

### 6.16 Stream gatherers

Stream gatherers are stable in Java 24 and available in Java 25, but **restricted**.

Use gatherers only when:

1. A custom intermediate stream operation is genuinely clearer than a loop.
2. The operation is reusable.
3. State management is explicit and tested.
4. Parallel behavior is understood or explicitly unsupported.
5. The code remains readable to maintainers.

Prefer an explicit loop when the operation is domain-heavy, stateful in a complex way, or easier to debug imperatively.

Rules:

1. Do not use gatherers for simple `map`, `filter`, `flatMap`, `limit`, `takeWhile`, or collector cases.
2. Do not hide business workflows inside a gatherer.
3. Avoid gatherers in hot paths without benchmark evidence.
4. Test empty, single-element, many-element, and cancellation/short-circuit cases.
5. Document whether parallel streams are supported.
6. Prefer named gatherers over inline complex lambdas.

### 6.17 Foreign Function & Memory API

The FFM API is stable since Java 22 and available in Java 25, but **highly restricted**.

Use FFM only for approved native interop where Java-only alternatives are insufficient.

Rules:

1. Keep FFM usage behind a narrow adapter boundary.
2. Never expose `MemorySegment`, `Arena`, native addresses, or layouts through normal domain APIs.
3. Use confined lifetimes and try-with-resources for arenas.
4. Validate all sizes, offsets, encodings, and native return codes.
5. Treat native calls as unsafe external boundaries.
6. Do not pass user-controlled data to native code without validation.
7. Do not use FFM for performance unless benchmarked against pure Java.
8. Do not mix FFM with `Unsafe` to work around safety checks.
9. Add platform-specific tests and fallback behavior.
10. Document OS/architecture/library assumptions.

### 6.18 Class-File API

The Class-File API is stable since Java 24 and available in Java 25, but **restricted to tooling**.
...
4. Controlled bytecode transformation.
5. Educational tooling.

Forbidden by default:

1. Business logic.
2. Runtime patching of production domain classes.
3. Security-sensitive bypasses.
4. Replacing normal Java source with generated bytecode without review.
5. Dynamic class generation when a normal class would work.

Rules:

1. Keep bytecode manipulation isolated.
2. Add golden tests for generated/parsed class files.
3. Avoid depending on internal class-file structures outside the API.
4. Do not combine with reflection/agents unless explicitly approved.

### 6.19 Markdown documentation comments

Markdown documentation comments are available, but **restricted**.

Rules:

1. Follow the repository documentation style.
2. Do not mix classic Javadoc and Markdown style randomly.
3. Use Markdown comments only when the Javadoc toolchain supports them.
4. Keep API docs focused on contract, not implementation noise.
5. Prefer examples that compile.
6. Avoid documentation that promises behavior not enforced by tests.

### 6.20 Key Derivation Function API

The KDF API is stable in Java 25, but **security-restricted**.

Rules:

1. Use only inside a reviewed cryptographic design.
2. Do not invent protocols.
3. Do not choose algorithms casually.
4. Preserve salt, context info, key length, and algorithm identifiers explicitly.
5. Do not log secrets, derived keys, shared secrets, private keys, or raw key material.
6. Prefer platform/security-team-approved crypto libraries and standards.
7. Include interoperability tests where another system consumes the derived key.
8. Use `SecureRandom` for required randomness.
9. Avoid static salts and hard-coded secrets.
10. Document the threat model and rotation story.

### 6.21 Key Encapsulation Mechanism API and post-quantum algorithms

Java 21 introduced KEM API and Java 24 added quantum-resistant ML-KEM / ML-DSA support. In Java 25, these remain **security-restricted**.

Rules:

1. Do not invent hybrid key exchange protocols.
2. Do not replace TLS behavior manually unless the project owns the protocol.
3. Use provider-supported algorithm names only.
4. Validate interoperability with counterparties.
5. Do not serialize keys using ad-hoc encodings.
6. Keep key lifecycle and destruction policy explicit.
7. Prefer standard TLS/JCA/JCE integration over manual cryptographic plumbing.

### 6.22 Preview/incubator traps in Java 25

The following are common traps for LLM agents:

| Tempting feature                              | Status in Java 25 | Strict rule                                                                             |
| --------------------------------------------- | ----------------- | --------------------------------------------------------------------------------------- |
| Primitive patterns in `instanceof` / `switch` | Preview           | Forbidden by default.                                                                   |
| Structured Concurrency                        | Fifth preview     | Forbidden by default. Use normal `ExecutorService` or explicitly approved preview.      |
| Stable Values                                 | Preview           | Forbidden by default. Use explicit lifecycle-managed lazy initialization.               |
| PEM Encodings API                             | Preview           | Forbidden by default. Use existing approved libraries or explicit parsing boundary.     |
| Vector API                                    | Incubator         | Forbidden by default. Use scalar code unless incubator use is approved and benchmarked. |
| JFR CPU-Time Profiling                        | Experimental      | Tooling only; do not add production code dependency.                                    |
...
Even on Java 25, do not rely on the platform default charset for durable data.
...
When using Java 11+ `HttpClient` in Java 25:
...
Records can be good DTOs in Java 25.
...
Java 25 warns when agents are loaded dynamically into a running JVM.
...
1. Java 25 supports runtime GC choices including G1, ZGC, and Generational ZGC.
...
### 29.1 Before editing code

The agent **MUST** identify:

1. Java baseline and build tool configuration.
2. Existing code style and formatter.
3. Existing test framework.
4. Existing exception model.
5. Existing logging framework.
6. Existing dependency injection pattern.
7. Existing concurrency model.
8. Existing serialization model.
9. Existing transaction boundaries.
10. Whether Java 25 stable-only rules apply or preview/incubator features are explicitly enabled.
...
2. Preserve public contracts unless explicitly asked to change them.
3. Preserve domain vocabulary.
4. Validate inputs at boundaries.
5. Keep invariants close to the data they protect.
6. Preserve exception causes.
7. Add tests for changed behavior.
8. Avoid unrelated formatting.
9. Avoid broad refactors.
10. Avoid using new Java 25 features unless they improve the model.

### 29.3 When using Java 25 features

For every Java 25-specific feature used, the agent **MUST** be able to explain:

1. Which feature was used.
2. Whether it is stable, restricted, preview, incubator, or experimental.
3. Why it improves clarity, correctness, safety, performance, or observability.
4. Why a simpler Java construct was not better.
5. Whether it affects serialization, public API, binary compatibility, reflection, frameworks, or tests.
6. Whether the build needs special flags.
7. Whether runtime/deployment changes are needed.

Restricted features require explicit justification in the implementation summary.

### 29.4 Before final response

The agent **MUST** summarize:

1. Files changed.
2. Behavior changed.
3. Java 25 features used and why.
4. Tests added or updated.
5. Compatibility impact.
6. Security impact.
7. Risks and assumptions.
8. Any preview/incubator/experimental usage.

---

## 30. Java 25 feature decision matrix for agents

| Situation                            | Preferred choice                                           | Avoid                                                 |
| ------------------------------------ | ---------------------------------------------------------- | ----------------------------------------------------- |
| Simple DTO/value carrier             | Record                                                     | Mutable POJO with no invariant; record for JPA entity |
| Closed finite domain alternatives    | Sealed interface + explicit implementations                | Open hierarchy with hidden invalid subtypes           |
| Mapping closed alternatives to value | Pattern switch                                             | Side-effect-heavy switch                              |
| Runtime type check                   | Pattern `instanceof`                                       | Cast-heavy code                                       |
| Ordered first/last/reversed contract | Sequenced collection                                       | Assuming `HashMap`/`HashSet` order                    |
| High-concurrency blocking I/O        | Virtual threads with resource bounds                       | Pooling virtual threads; CPU-bound virtual threads    |
| Request-scoped immutable context     | Explicit parameter first; scoped value only when justified | `ThreadLocal` leaks; global mutable context           |
| Ignored local/pattern variable       | `_` only when intentionally ignored                        | Meaningless names that hide important data            |
| Small script or reproducer           | Compact source file / instance main                        | Compact source file in production service             |
| Many imports for quick script        | `import module` if intentional                             | Module imports in mature production source            |
| Pre-`super` constructor validation   | Flexible constructor body                                  | Side effects before construction                      |
| Custom stream intermediate operation | Gatherer only if reusable and tested                       | Obscure stateful stream magic                         |
| Native interop                       | FFM behind adapter boundary                                | `Unsafe`, raw JNI, scattered native calls             |
| Bytecode tooling                     | Class-File API behind tooling boundary                     | Runtime bytecode hacks in business logic              |
| Lazy initialization                  | Explicit holder/supplier/cache                             | Stable Values preview by default                      |
| Structured task fan-out              | Normal executor; preview only with approval                | Java 25 `StructuredTaskScope` by accident             |
| Primitive type pattern matching      | Explicit primitive switch/conversion                       | Java 25 primitive patterns preview                    |
| Crypto key derivation                | Reviewed KDF API usage                                     | Ad-hoc hashing/secrets derivation                     |
| PEM parsing                          | Existing approved library or explicit parser boundary      | Java 25 PEM preview by default                        |
| SIMD optimization                    | Clear scalar code, benchmark first                         | Vector API incubator by default                       |
| Startup optimization                 | Deployment-level AOT config                                | Source code coupled to AOT cache                      |

---

## 31. Migration rules: Java 21 to Java 25

When migrating Java 21 code to Java 25, the agent **MUST** treat the migration as compatibility-sensitive engineering work, not syntax modernization.

Recommended order:

1. Confirm Java 25 runtime, compiler, build plugins, CI image, container image, code coverage agent, annotation processors, and static analysis tools.
2. Compile existing code with `--release 25` without source changes.
3. Run full tests before modernization.
4. Fix dependency incompatibilities before using new Java 25 features.
5. Audit startup scripts for removed/deprecated JVM flags.
6. Audit `SecurityManager`, `Unsafe`, JNI, dynamic agents, reflection, and `--add-opens` usage.
7. Audit GC flags, especially old ZGC and Shenandoah assumptions.
8. Audit 32-bit x86 deployment assumptions.
9. Audit serialization and reflection behavior before converting classes to records/sealed hierarchies.
10. Modernize code only where the new feature strengthens the model.
11. Add tests around every behavior touched by migration.
12. Benchmark before changing GC, AOT, virtual-thread, scoped-value, or stream-gatherer usage.

Allowed migration improvements:

1. Use `List.copyOf`, records, sealed types, and pattern switch where they encode real invariants.
2. Use virtual threads for blocking I/O only after load testing and resource bounding.
3. Replace `ThreadLocal` request context with scoped values only when the lifetime is bounded and immutable.
4. Use flexible constructor bodies for clear pre-`super` validation.
5. Use stream gatherers only for reusable, tested stream operations.
6. Use FFM only to replace brittle native interop behind a narrow adapter.
7. Use KDF/KEM APIs only under security review.
8. Use AOT/JFR runtime features as deployment/diagnostic improvements, not as source-level assumptions.
...
4. Replacing all `ThreadLocal` usage with scoped values without lifecycle analysis.
5. Replacing all loops with streams or gatherers.
6. Adding compact source files to production modules.
7. Adding module imports to mature production code without style approval.
8. Adding preview/incubator features because they are “Java 25”.
9. Changing serialized DTO field names through record component renaming.
10. Changing transaction boundaries during syntax migration.
11. Changing exception behavior during syntax migration.
12. Changing public API signatures for cosmetic reasons.
13. Reformatting the entire repository without explicit request.
14. Adding FFM, JNI, agents, or bytecode generation as convenience shortcuts.
15. Tuning GC/AOT flags without measurement and rollback plan.
...
11. Use scoped values only if framework integration is understood; do not bypass request context/security context mechanisms blindly.
12. Do not use compact source files for Spring Boot applications.
...
11. Do not use sealed/entity hierarchies unless the ORM mapping is explicitly verified.
12. Do not use flexible constructor bodies to perform persistence or dependency lookup.
...
8. Test record/sealed/pattern-based DTO changes with serialization snapshots.
9. Do not expose compact-source/example models as external API contracts.
...
6. Prefer records over Lombok DTOs only when serialization/framework compatibility is verified.
...
11. Check preview/incubator flags are absent unless intentionally approved.
12. Check AOT/JFR/GC options separately from source compatibility.

---

## 33. Bad patterns forbidden for LLM-generated Java 25 code
...
```java
catch (Exception _) { return null; } // unnamed variable used to hide swallowed exception
```
...
```java
case int i -> ... // primitive pattern preview in Java 25, forbidden by default
```

```java
StructuredTaskScope.open(); // preview in Java 25, forbidden by default unless approved
```

```java
StableValue.of(); // preview in Java 25, forbidden by default unless approved
```

```java
import module java.base; // forbidden by default in production/shared code
```

```java
void main() { } // compact/instance main style, forbidden by default in production services
```

```java
MemorySegment.ofAddress(address); // unsafe native boundary without adapter/review
```
...
```java
sun.misc.Unsafe.getUnsafe(); // forbidden new usage
```

```java
System.setSecurityManager(...); // Security Manager is permanently disabled
```
...
A Java 25 change is acceptable only if the reviewer can answer “yes” to the relevant items.

### Compatibility

- [ ] Code compiles with `--release 25`.
- [ ] No Java >25 APIs/features are used.
- [ ] No Java 25 preview/incubator/experimental features are used unless explicitly approved.
- [ ] Build/test runtime is Java 25-compatible.
- [ ] Public API compatibility is preserved or intentionally changed.
- [ ] Removed/deprecated JDK flags and APIs are not introduced.
...
- [ ] Primitive patterns are absent unless preview usage is approved.
...
- [ ] Scoped values, if used, have bounded immutable context and clear ownership.
- [ ] Unnamed variables/patterns are used only for intentionally ignored values.
- [ ] Module imports are absent from production/shared code unless style-approved.
- [ ] Compact source files are absent from production services/libraries.
- [ ] Flexible constructor bodies contain only safe validation/preparation.
- [ ] Stream gatherers are justified, named, and tested.
- [ ] FFM usage is isolated behind an adapter and reviewed.
- [ ] Class-File API usage is tooling-only and tested.
- [ ] KDF/KEM usage has security review.
- [ ] Preview/incubator/experimental features are absent unless explicitly approved.
...
- [ ] Reflection/agent/native usage is justified.
- [ ] Sensitive data is masked in logs.
- [ ] `SecurityManager`, `Unsafe`, and `--add-opens` are not used as security shortcuts.
...
- [ ] Virtual-thread usage does not create unbounded downstream pressure.
- [ ] Scoped values do not hide business invariants.
...
- [ ] Modern syntax does not obscure simple logic.

---

## 35. Prompt snippet for LLM code agents

Use this snippet when asking an LLM agent to implement Java 25 code:

```text
You are modifying a strict Java 25 codebase.

Follow strict-coding-standards__java25.md as a binding implementation contract.

Hard constraints:
- Compile with Java 25 using --release 25.
- Do not use Java >25 features.
- Do not use Java 25 preview/incubator/experimental features unless explicitly requested.
- Do not introduce dependencies, framework changes, concurrency model changes, public API changes, transaction changes, persistence schema changes, serialization changes, native interop, bytecode manipulation, dynamic agents, AOT assumptions, or broad refactors unless required.
...
- Use Java 25 stable features only when they improve clarity or encode a real invariant.
- Treat records, record patterns, pattern switch, sequenced collections, sealed types, virtual threads, scoped values, unnamed variables, module imports, compact source files, flexible constructor bodies, stream gatherers, FFM, Class-File API, and KDF/KEM as restricted tools, not default choices.
- Never pool virtual threads.
- Bound external resources even when using virtual threads.
- Keep scoped values immutable, bounded, and hidden behind a small context boundary.
- Do not use primitive patterns, StructuredTaskScope, Stable Values, PEM Encodings API, Vector API, or JFR experimental APIs unless explicitly approved with the required flags.
...
2. Behavior changed.
3. Java 25 features used and why.
4. Any restricted/preview/incubator/experimental feature usage.
5. Tests added/updated.
6. Risks, assumptions, and compatibility notes.
```
...
This document is intentionally based on stable Java 25 / Java SE 25 behavior and conservative production engineering practice.

Primary references:

1. Oracle JDK 25 Documentation — https://docs.oracle.com/en/java/javase/25/
2. Oracle JDK 25 API Documentation — https://docs.oracle.com/en/java/javase/25/docs/api/
3. Oracle JDK 25 Release Notes — https://www.oracle.com/java/technologies/javase/25-relnote-issues.html
4. OpenJDK JDK 25 Project Page — https://openjdk.org/projects/jdk/25/
5. OpenJDK JEPs in JDK 25 since JDK 21 — https://openjdk.org/projects/jdk/25/jeps-since-jdk-21
6. JEP 456: Unnamed Variables and Patterns — https://openjdk.org/jeps/456
7. JEP 454: Foreign Function & Memory API — https://openjdk.org/jeps/454
8. JEP 467: Markdown Documentation Comments — https://openjdk.org/jeps/467
9. JEP 484: Class-File API — https://openjdk.org/jeps/484
10. JEP 485: Stream Gatherers — https://openjdk.org/jeps/485
11. JEP 506: Scoped Values — https://openjdk.org/jeps/506
12. JEP 510: Key Derivation Function API — https://openjdk.org/jeps/510
13. JEP 511: Module Import Declarations — https://openjdk.org/jeps/511
14. JEP 512: Compact Source Files and Instance Main Methods — https://openjdk.org/jeps/512
15. JEP 513: Flexible Constructor Bodies — https://openjdk.org/jeps/513
16. JEP 519: Compact Object Headers — https://openjdk.org/jeps/519
17. JEP 521: Generational Shenandoah — https://openjdk.org/jeps/521
18. JEP 470: PEM Encodings of Cryptographic Objects (Preview) — https://openjdk.org/jeps/470
19. JEP 502: Stable Values (Preview) — https://openjdk.org/jeps/502
20. JEP 505: Structured Concurrency (Fifth Preview) — https://openjdk.org/jeps/505
21. JEP 507: Primitive Types in Patterns, instanceof, and switch (Third Preview) — https://openjdk.org/jeps/507
22. JEP 508: Vector API (Tenth Incubator) — https://openjdk.org/jeps/508
23. JEP 486: Permanently Disable the Security Manager — https://openjdk.org/jeps/486
24. JEP 503: Remove the 32-bit x86 Port — https://openjdk.org/jeps/503
25. JEP 471: Deprecate the Memory-Access Methods in sun.misc.Unsafe for Removal — https://openjdk.org/jeps/471
26. JEP 472: Prepare to Restrict the Use of JNI — https://openjdk.org/jeps/472
27. JEP 498: Warn upon Use of Memory-Access Methods in sun.misc.Unsafe — https://openjdk.org/jeps/498
28. Oracle Secure Coding Guidelines for Java SE — https://www.oracle.com/java/technologies/javase/seccodeguide.html
29. Google Java Style Guide — https://google.github.io/styleguide/javaguide.html
30. Oracle Code Conventions for the Java Programming Language — https://www.oracle.com/java/technologies/javase/codeconventions-contents.html
...
1. Compatible with Java 25.
2. Stable, not preview/incubator/experimental.
3. Easy to review.
4. Explicit about failure.
5. Explicit about resource ownership.
6. Explicit about security boundaries.
7. Aligned with existing repository conventions.
8. Boring enough that the next maintainer will understand it quickly.

If a modern Java 25 feature makes the code shorter but less obvious, **do not use it**.

If a modern Java 25 feature encodes a real invariant more strongly, **use it carefully and explain why**.
